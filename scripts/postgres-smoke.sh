...
business_pilot_unsafe_group_count="$(business_pilot_reader_psql -At -c "SELECT COUNT(*) FROM gongmcp_profile_call_fact_summary_sanitized($business_pilot_profile_id, '$business_pilot_profile_sha', 'deal_stage', '', '', '', '', '', 5)" | tr -d '[:space:]')"
if [ "$business_pilot_unsafe_group_count" != "0" ]; then
  echo "business-pilot scoped reader sanitized profile summary unexpectedly grouped by CRM value" >&2
  exit 1
fi
...
grep -q 'gongmcp_profile_call_fact_summary' /tmp/gongctl-postgres-business-pilot-reader-extra-grant.txt
