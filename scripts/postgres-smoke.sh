...
if grep -q 'source_path\|source_sha256\|canonical_sha256\|imported_by\|canonical_json\|evidence' /tmp/gongctl-postgres-business-pilot-reader-active-profile-sanitized.json; then
  echo "business-pilot scoped reader sanitized active-profile helper exposed source metadata, canonical hash, canonical JSON, or field evidence" >&2
  exit 1
fi
...
grep -q 'gongmcp_profile_call_fact_cache_meta' /tmp/gongctl-postgres-business-pilot-reader-extra-grant.txt
grep -q 'gongmcp_active_business_profile' /tmp/gongctl-postgres-business-pilot-reader-extra-grant.txt
docker compose -p "$PROJECT" -f "$COMPOSE_FILE" exec -T postgres psql -U gongctl -d gongctl -v ON_ERROR_STOP=1 -c "REVOKE EXECUTE ON FUNCTION gongmcp_profile_call_fact_cache(bigint, text) FROM gongmcp_business_pilot_reader; REVOKE EXECUTE ON FUNCTION gongmcp_profile_call_fact_cache_meta(bigint, text) FROM gongmcp_business_pilot_reader; REVOKE EXECUTE ON FUNCTION gongmcp_profile_call_fact_cache_sanitized(bigint, text) FROM gongmcp_business_pilot_reader; REVOKE EXECUTE ON FUNCTION gongmcp_active_business_profile() FROM gongmcp_business_pilot_reader" >/dev/null
docker compose -p "$PROJECT" -f "$COMPOSE_FILE" exec -T postgres psql -U gongctl -d gongctl -v ON_ERROR_STOP=1 </tmp/gongctl-postgres-business-pilot-reader-grants-gongctl.sql >>/tmp/gongctl-postgres-business-pilot-reader-create.txt 2>&1
