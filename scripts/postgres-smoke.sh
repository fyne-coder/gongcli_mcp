...
grep -q 'GRANT EXECUTE ON FUNCTION public.gongmcp_profile_call_fact_cache_sanitized(bigint, text)' /tmp/gongctl-postgres-business-pilot-reader-grants.sql
grep -q 'GRANT EXECUTE ON FUNCTION public.gongmcp_profile_call_fact_cache_sanitized(bigint, text)' /tmp/gongctl-postgres-business-pilot-reader-grants-gongctl.sql
grep -q 'GRANT EXECUTE ON FUNCTION public.gongmcp_profile_call_fact_summary' /tmp/gongctl-postgres-business-pilot-reader-grants.sql
grep -q 'GRANT EXECUTE ON FUNCTION public.gongmcp_profile_call_fact_summary' /tmp/gongctl-postgres-business-pilot-reader-grants-gongctl.sql
for generated_sql in /tmp/gongctl-postgres-business-pilot-reader-grants.sql /tmp/gongctl-postgres-business-pilot-reader-grants-gongctl.sql; do
  grep -q 'Apply to a fresh NOINHERIT role' "$generated_sql"
  grep -q 'not an analyst SQL login' "$generated_sql"
  grep -q 'minimized operational metadata' "$generated_sql"
  grep -q 'business-pilot scoped reader for profile-backed lifecycle analysis' "$generated_sql"
...
SQL
SH
docker compose -p "$PROJECT" -f "$COMPOSE_FILE" exec -T postgres psql -U gongctl -d gongctl -v ON_ERROR_STOP=1 </tmp/gongctl-postgres-business-pilot-reader-grants-gongctl.sql >>/tmp/gongctl-postgres-business-pilot-reader-create.txt 2>&1
...
business_pilot_direct_profile_identifier_count="$(business_pilot_reader_psql -At -c "SELECT COUNT(*) FROM gongmcp_profile_call_fact_cache_sanitized($business_pilot_profile_id, '$business_pilot_profile_sha') WHERE call_id <> '' OR title <> ''" | tee /tmp/gongctl-postgres-business-pilot-reader-profile-cache-sanitized.txt | tr -d '[:space:]')"
if [ "$business_pilot_direct_profile_identifier_count" != "0" ]; then
  echo "business-pilot scoped reader sanitized profile cache returned identifier-bearing rows" >&2
  exit 1
fi
business_pilot_direct_profile_fact_count="$(business_pilot_reader_psql -At -c "SELECT COUNT(*) FROM gongmcp_profile_call_fact_cache_sanitized($business_pilot_profile_id, '$business_pilot_profile_sha') WHERE started_at <> '' AND lifecycle_bucket <> ''" | tee -a /tmp/gongctl-postgres-business-pilot-reader-profile-cache-sanitized.txt | tr -d '[:space:]')"
if [ "$business_pilot_direct_profile_fact_count" -lt 1 ]; then
  echo "business-pilot scoped reader sanitized profile cache returned no non-identifier fact rows" >&2
  exit 1
fi
...
grep -q 'gongmcp_profile_call_fact_cache' /tmp/gongctl-postgres-business-pilot-reader-extra-grant.txt
docker compose -p "$PROJECT" -f "$COMPOSE_FILE" exec -T postgres psql -U gongctl -d gongctl -v ON_ERROR_STOP=1 -c "REVOKE EXECUTE ON FUNCTION gongmcp_profile_call_fact_cache(bigint, text) FROM gongmcp_business_pilot_reader" >/dev/null
...
GONG_DATABASE_URL="$WRITER_URL" go run ./cmd/gongctl sync read-model --rebuild >/tmp/gongctl-postgres-reader-missing-transcripts-function-grant-drift-repaired.json
grep -q '"status": "rebuilt"' /tmp/gongctl-postgres-reader-missing-transcripts-function-grant-drift-repaired.json
...
echo "synthetic business-pilot scoped reader sanitized profile-cache output: /tmp/gongctl-postgres-business-pilot-reader-profile-cache-sanitized.txt"
echo "synthetic business-pilot scoped reader sanitized profile-cache identifier-bearing row count: ${business_pilot_direct_profile_identifier_count:-unknown}"
echo "synthetic business-pilot scoped reader sanitized profile-cache non-identifier fact row count: ${business_pilot_direct_profile_fact_count:-unknown}"
