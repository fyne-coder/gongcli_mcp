...
grep -q 'GRANT SELECT ("parties_count", "started_at") ON TABLE public."calls"' /tmp/gongctl-postgres-business-pilot-reader-grants.sql
grep -q 'GRANT SELECT ("parties_count", "started_at") ON TABLE public."calls"' /tmp/gongctl-postgres-business-pilot-reader-grants-gongctl.sql
...
for generated_sql in /tmp/gongctl-postgres-business-pilot-reader-grants.sql /tmp/gongctl-postgres-business-pilot-reader-grants-gongctl.sql; do
  ...
  grep -q 'minimized call IDs/titles' "$generated_sql"
...
business_pilot_direct_profile_identifier_count="$(business_pilot_reader_psql -At -c "SELECT COUNT(*) FROM gongmcp_profile_call_fact_cache($business_pilot_profile_id, '$business_pilot_profile_sha') WHERE call_id <> '' OR title <> ''" | tr -d '[:space:]')"
if [ "$business_pilot_direct_profile_identifier_count" -lt 1 ]; then
  echo "business-pilot scoped reader direct profile cache residual-risk smoke did not observe identifier-bearing rows" >&2
  exit 1
fi
...
echo "synthetic business-pilot scoped reader direct profile-cache service-credential identifier count observed: ${business_pilot_direct_profile_identifier_count:-unknown}"
