...
interface in `server.go`. SQLite is the complete/default backend; Postgres is a
reviewed shared-deployment slice wired through `cmd/gongmcp/main.go` and
`internal/store/postgres`. Stdio is the default transport; HTTP mode is a
minimal private-pilot transport wrapper around the same request handler.

Rules:

- read from the configured cache/store surfaces; do not call Gong directly
- keep tools read-only
- require `--db` at the `cmd/gongmcp` boundary for SQLite, or a Postgres URL
  from `GONG_DATABASE_URL` / `DATABASE_URL` when no `--db` is provided
...
- reject Postgres `all-readonly` until full parity is reviewed; see
  `cmd/gongmcp/main.go` and `docs/postgres-parity.md`
- keep facade visible tools and hidden routed tools in sync through
  `FacadeOperations`, `FacadeToolNames`, `FacadeHiddenRoutedToolNames`, and
  `ExpandToolPresetFacadeRoutedTools`
...
- keep profile-aware tools tied to imported profile state and fresh profile
  caches; read-only MCP should report stale state instead of rebuilding it
