		...
		case "/mcp":
			...
			if !opts.MissingChallenge {
				if opts.FalseBearerChallenge {
					w.Header().Set("WWW-Authenticate", fmt.Sprintf(`NotBearer resource_metadata="%s/.well-known/oauth-protected-resource/mcp", scope="gongmcp/read"`, baseURL))
				} else {
					w.Header().Set("WWW-Authenticate", fmt.Sprintf(`Bearer resource_metadata="%s/.well-known/oauth-protected-resource/mcp", scope="gongmcp/read"`, baseURL))
				}
			}
