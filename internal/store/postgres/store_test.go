...
func TestPostgresBusinessAnalysisEvidenceFunctionKeepsFTSQueryRequired(t *testing.T) {
	t.Parallel()

	if strings.Contains(postgresBusinessAnalysisFunctionsSQL, "COALESCE(search_text, '') = '' OR ts.search_vector @@ q.query") {
		t.Fatal("gongmcp_business_analysis_evidence must not accept empty search_text; seedless theme discovery uses a separate function")
	}
	if !strings.Contains(postgresBusinessAnalysisFunctionsSQL, "WHERE ts.search_vector @@ q.query") {
		t.Fatal("gongmcp_business_analysis_evidence should keep the FTS predicate query-required")
	}
	for _, want := range []string{
