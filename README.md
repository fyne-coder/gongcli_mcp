...
Transcript evidence provenance is configurable at MCP server launch with `--transcript-evidence-provenance=redacted|alias|raw` or `GONGMCP_TRANSCRIPT_EVIDENCE_PROVENANCE`. The default `redacted` mode omits call and speaker identifiers. `alias` mode returns stable non-raw `call_ref` and `speaker_ref` values for artifact citation without exposing Gong IDs. `raw` mode returns raw `call_id` and `speaker_id` and should be used only for local operator workflows.
