...
docker run --rm -v "$HOME/gongctl-data:/data" ghcr.io/fyne-coder/gongcli_mcp/gongctl:v0.5.4 sync status --db /data/gong.db
```

The `v0.5.4` image references require the `v0.5.4` tag workflow to have
...
```bash
docker run --rm -i --network none -v "$HOME/gongctl-data:/data:ro" ghcr.io/fyne-coder/gongcli_mcp/gongmcp:v0.5.4 --db /data/gong.db --tool-preset business-pilot
```
