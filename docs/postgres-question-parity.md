...
| Pull Gong-native call briefs, highlights, outlines, or CRM entity briefs. | supported with caveats for operator capture; blocked for MCP | `gongctl sync calls --include-highlights` (raw-capture only; see [Gong-native enrichment review](gong-native-enrichment-review.md)) | Phase 13k adds opt-in raw capture for AI Highlights/brief/next-step content via `contentSelector.exposedFields.highlights=true`. No MCP tool exposure yet; entity brief and topics/trackers/comments are still queued. |
