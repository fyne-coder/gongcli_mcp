...
| Pull Gong-native call briefs, highlights, outlines, or CRM entity briefs. | supported with caveats for operator capture/read model; blocked for MCP | `gongctl sync calls --include-highlights` plus internal `call_ai_highlights` Postgres read model; see [Gong-native enrichment review](gong-native-enrichment-review.md) | Highlights are captured opt-in and typed into a governed Postgres read model, including redacted serving DB filtering. No MCP operation exposes them yet; entity brief and topics/trackers/comments are still queued. |
