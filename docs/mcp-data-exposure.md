...
- Postgres `get_call` remains a record-reference tool and should be enabled
  ...
  policy for the narrowed `governance-search` preset. Client-facing Postgres
  deployments with customer exclusions should use a physically redacted serving
  DB generated by `gongctl governance refresh-serving-db`.
...
| `summarize_call_facts`, `summarize_calls_by_lifecycle` | Safe-default | Aggregate | Return rates, counts, classification logic, or allowlisted business dimensions only | Group labels can still expose deployment-specific terminology |
...
| `analyze_late_stage_crm_signals`, `list_lifecycle_buckets`, `compare_lifecycle_crm_fields` | Restricted | Aggregate | Return rates, counts, classification logic, or allowlisted business dimensions only | Business groupings can reveal deployment-specific CRM structure |
...
| `gong_query` | Bounded query operations | `query.call_count` -> internal `call_count` handler; `query.dimension_counts` -> internal `dimension_counts` handler; `query.calls` -> `search_calls_by_filters` (fallback `search_calls`); `query.transcript_segments` -> `search_transcript_segments` (fallback `search_transcripts_by_filters`); `query.scorecards` -> `list_scorecards`; `query.scorecard_detail` -> `get_scorecard` |
...
Call titles are treated as customer metadata, but they are visible by default
in title-bearing MCP surfaces when the backend has a title and policy allows
it. Use `field_profile=limited` for request-level suppression, or add
`hide_call_titles` to `--policy-switches` /
`GONGMCP_POLICY_SWITCHES` for process-level suppression. Keep using
`call_ref`, date/start time, duration, Gong generated brief/highlight rows,
and transcript quote evidence as the stable client path when titles are
hidden.
...
1. The selected MCP operation and store surface must be title-bearing.
2. The request must not use `field_profile=limited`.
3. The MCP runtime must not enable `hide_call_titles` in `--policy-switches`
   or `GONGMCP_POLICY_SWITCHES`.

For trusted SQLite analyst sessions, use a title-bearing preset such as
`all-readonly`; titles are returned by default:
...
For Postgres sessions where call titles should be visible, use a reviewed
title-bearing surface against a physically redacted serving DB:
...
Do not include `hide_call_titles` in `GONGMCP_POLICY_SWITCHES`. Use
`field_profile=limited` only for requests where call titles should be hidden.
...
internal mode, call titles are visible by default unless policy suppresses
them; explicit include flags can expose raw call IDs that remain in the
physically redacted serving DB.
...
group labels and field names can still reveal deployment-specific CRM structure.
...
- Accept a bounded `call_filter` with documented fields only:
...
- `dimension_filters` may filter backed business-analysis fields and
  dimensions. The default disallow list is empty; deployments can add a policy
  disallow list later if needed. String-backed dimensions use `equals` or `in`
  over exact values; numeric dimensions such as `duration_seconds` also support
  `gte`, `lte`, and `between`. Multiple entries are combined with AND
  ...
  they narrow the cohort without changing the normal output policy behavior.
...
- For SQLite analyst sessions with customer exclusions, use a physically
  governance-filtered DB. For Postgres analyst sessions with customer
  exclusions, use a physically redacted serving DB and a scoped reader role.
  Raw/source DB AI governance mode intentionally fails closed for these
  aggregate cohort tools because their counts and slices are not recomputed
  over a filtered call set.
