...
```bash
docker run --rm ghcr.io/fyne-coder/gongcli_mcp/gongctl:v0.6.5 version
```

The `v0.6.5` examples require the matching Git tag workflow to have published
...
- `ghcr.io/fyne-coder/gongcli_mcp/gongctl:v0.6.5`
- `ghcr.io/fyne-coder/gongcli_mcp/gongmcp:v0.6.5`
- `gongctl:local`
- `gongctl:mcp-local`
...
```bash
docker run --rm ghcr.io/fyne-coder/gongcli_mcp/gongmcp:v0.6.5 --list-tool-presets
```
...
Outside this Docker example, do not bind `gongmcp --http` to a non-loopback
interface unless it is on a private or management network behind that gateway.
...
Hosted ChatGPT/OpenAI and Claude/Anthropic connector paths are initiated from
provider infrastructure, so private-network-only, VPN-only, localhost, and
stdio-only endpoints will not work for those hosted flows.
...
Replace `/path/to/gongctl-data` with the absolute path on that machine. If
