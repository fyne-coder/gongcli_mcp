...
| Security reviewer | data boundary, support policy, governance list | tool allowlist, restricted-data filter, support-bundle policy, audit expectations | strict pilot boundary is approved |
...
- Tool allowlist profile: `strict-business-pilot`, `operator-smoke`, or
  `analyst-expansion`.
...
IT teams do not compare raw comma-separated lists by eye.

| Profile | Tools | Use |
| --- | --- | --- |
| `strict-business-pilot` | `get_sync_status,summarize_call_facts,summarize_calls_by_lifecycle,rank_transcript_backlog` | default business-user lane |
...
| `analyst-expansion` | customer-approved subset from [MCP data exposure](mcp-data-exposure.md) | trusted analyst sessions after sponsor approval |

Do not expose `operator-smoke` or `analyst-expansion` as the default business
user allowlist.
