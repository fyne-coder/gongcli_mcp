...
| Analyst presence dimensions | SQLite dimension helpers derive from cached JSON/context at read time | complete for materialized Postgres presence buckets | Persona/title dimensions now map cached party and Contact/Lead title fields into coarse role buckets while keeping raw title strings out of analyst outputs; Postgres still uses `call_facts.participant_title_present` only as a fallback when raw title text is unavailable. Loss-reason dimensions remain presence-only through `call_facts.loss_reason_present`. | postgres-native hardening | Phase 13f | `TestPostgresBusinessAnalysisPhase5BMatchesSQLiteRepresentativeSlice`; `TestPostgresBusinessAnalysisPersonaBucketsMatchSQLiteRepresentativeSlice`; `scripts/postgres-load-smoke.sh` |
...
| Governance filtered DB export | SQLite physical filtered copy plus `VACUUM INTO` | SQLite export remains file-copy based; Postgres has prepared-policy governance for narrowed raw/source `governance-search` and `gongctl governance refresh-serving-db` for physically redacted MCP serving databases | Source-DB RLS remains a follow-up; client-facing Postgres customer exclusions should use a redacted serving DB plus scoped reader grants | postgres-native equivalent | Phase 4 | restricted synthetic account absent from governed MCP search outputs; `scripts/postgres-serving-db-analyst-smoke.sh`; `scripts/postgres-ga-acceptance-smoke.sh` |
