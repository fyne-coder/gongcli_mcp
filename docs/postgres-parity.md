...
| `analyst-core` MCP preset | Postgres-specific starter surface | complete for core/profile/lifecycle/CRM-context inventory tools | Exposes only implemented Postgres analyst starter tools and keeps raw CRM values out of reader output | intentionally narrower than SQLite `analyst` | Phase 5a | analyst-core tools/list and CRM inventory smoke |
...
| Business-analysis calls/evidence | SQLite `business_analysis.go` helpers | queued after Phase 5a | Equivalent read APIs over normalized context/facts/transcripts | must match | Phase 5b | business-analysis parity fixtures |
