...
| `analyst-core` MCP preset | Postgres-specific starter surface | complete for core/profile/lifecycle/CRM-context inventory and scorecard inventory tools | Exposes only implemented Postgres analyst starter tools and keeps raw CRM values/raw settings payloads out of reader output | intentionally narrower than SQLite `analyst` | Phase 5a/5c | analyst-core tools/list, CRM inventory, and scorecard inventory smoke |
