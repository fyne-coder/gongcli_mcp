...
| `analyst-core` MCP preset | Postgres-specific starter surface | complete for core/profile/lifecycle/CRM-context inventory, scorecard inventory, and aggregate scorecard activity tools | Exposes only implemented Postgres analyst starter tools and keeps raw CRM values/raw settings payloads/raw scorecard activity payloads out of reader output | intentionally narrower than SQLite `analyst` | Phase 5a/5c/5d | analyst-core tools/list, CRM inventory, scorecard inventory, and scorecard activity smoke |
...
| CRM schema/settings inventory | SQLite `crm_integrations`, schema, settings, scorecards | partial for Postgres: CRM object/field inventory plus Gong settings and scorecard inventory complete; broader settings/catalog parity still queued | Same cached metadata read surfaces without raw CRM values or raw settings payloads | must match | Phase 5a/5c | inventory/query tests; `scripts/postgres-smoke.sh` |
